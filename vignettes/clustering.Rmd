...
grp_dist <- cluster_network(net, k = 2, cluster_by = "ward.D2") #Kamila4: Error in cluster_network(net, k = 2, cluster_by = "ward.D2") : unused arguments (k = 2, cluster_by = "ward.D2")
grp_dist
```
...
Both return a `netobject_group` — a list of `netobject`s with clustering info accessible via `attr(grp_mmm, "clustering")`:
...
ch <- cluster_choice(net, k = 2:5,
                     ...
                     method = c("pam", "ward.D2")) #Kamila5: Error in cluster::pam(dist_mat, diss = TRUE, k = k) : NA values in the dissimilarity matrix not allowed.
